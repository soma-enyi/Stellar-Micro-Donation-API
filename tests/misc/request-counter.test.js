...
describe('RequestCounter', () => {
  ...
  describe('increment', () => {
    test('should start count at 1 when new API key', () => {
      const count = counter.increment('test-key');
      expect(count).toBe(1);
    });

    test('should increment count when subsequent requests', () => {
      counter.increment('test-key');
      const count = counter.increment('test-key');
      expect(count).toBe(2);
    });

    test('should maintain separate counts when different API keys', () => {
    ...
    test('should reset count when window expires', (done) => {
      counter.increment('test-key');
      counter.increment('test-key');
  ...
  describe('getCount', () => {
    test('should return 0 when unknown API key', () => {
      expect(counter.getCount('unknown-key')).toBe(0);
    });

    test('should return current count when known API key', () => {
      counter.increment('test-key');
      counter.increment('test-key');
      expect(counter.getCount('test-key')).toBe(2);
    });

    test('should return 0 when window expires', (done) => {
      counter.increment('test-key');
