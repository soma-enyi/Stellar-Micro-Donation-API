...
describe('Stellar Time-Bound Transactions', () => {
  ...
  describe('Test 1: Successful donation within a valid window', () => {
    ...
    it('should accept donation with only validAfter (no upper bound)', async () => {
      const now = Math.floor(Date.now() / 1000);
      const validAfter = now - 3600; // 1 hour ago
    ...
    it('should accept donation with only validBefore (no lower bound)', async () => {
      const now = Math.floor(Date.now() / 1000);
      const validBefore = now + 3600; // 1 hour from now
  ...
  describe('Test 2: Immediate 400 error when validAfter > validBefore', () => {
    ...
    it('should provide clear error message for constraint violation', async () => {
      const now = Math.floor(Date.now() / 1000);
  ...
  describe('Test 3: Transaction failure when window has expired', () => {
    ...
    it('should simulate transaction failure when current time is after validBefore', async () => {
      const now = Math.floor(Date.now() / 1000);
    ...
    it('should correctly validate time bounds with edge case timestamps', async () => {
      const now = Math.floor(Date.now() / 1000);
  ...
  describe('Timebounds Storage and Audit', () => {
    ...
    it('should allow querying transactions with expired time bounds', async () => {
      const now = Math.floor(Date.now() / 1000);
  ...
  describe('Timebounds with other donation features', () => {
    ...
    it('should work with notes and tags', async () => {
      const now = Math.floor(Date.now() / 1000);
      const validAfter = now - 3600;
      const validBefore = now + 3600;
  ...
  describe('Clock skew and time bounds edge cases', () => {
    ...
    it('should handle rapid sequential donations with overlapping time windows', async () => {
      const now = Math.floor(Date.now() / 1000);
