...
describe('RBAC Middleware - Authorization Tests', () => {
  ...
  describe('Single Permission Check', () => {
    ...
    test('should allow user when correct permission', () => {
  ...
  describe('Any Permission Check', () => {
    test('should allow when user has any of the required permissions', () => {
      req.user = { id: '2', role: ROLES.USER };
    ...
    test('should block when user has none of the permissions', () => {
      req.user = { id: 'guest', role: ROLES.GUEST };
      const middleware = checkAnyPermission([
        PERMISSIONS.DONATIONS_CREATE,
        PERMISSIONS.WALLETS_DELETE
  ...
  describe('All Permissions Check', () => {
    ...
    test('should block when user is missing any permission', () => {
      req.user = { id: '2', role: ROLES.USER };
      const middleware = checkAllPermissions([
        PERMISSIONS.DONATIONS_CREATE,
        PERMISSIONS.WALLETS_DELETE
    ...
    test('should allow admin when all permissions', () => {
      req.user = { id: '1', role: ROLES.ADMIN };
      const middleware = checkAllPermissions([
        PERMISSIONS.DONATIONS_CREATE,
        PERMISSIONS.WALLETS_DELETE,
  ...
  describe('User Role Attachment', () => {
    test('should attach admin role when admin API key', async () => {
    ...
    test('should attach user role when regular API key', async () => {
      validateApiKey.mockResolvedValue({
        id: 2,
        role: ROLES.USER,
        name: 'User Key',
    ...
    test('should attach guest role when no API key', async () => {
      const middleware = attachUserRole();
