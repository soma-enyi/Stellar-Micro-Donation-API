...
describe('Suspicious Pattern Middleware Integration', () => {
  ...
  describe('Request Processing', () => {
    ...
    it('should track donation patterns when success', async () => {
      await request(app)
        .post('/api/v1/donations/send')
        .send({ amount: 10, senderId: 'SENDER', receiverId: 'RECEIVER' });
  ...
  describe('Error Handling', () => {
    it('should not crash when malformed request body', async () => {
      const response = await request(app)
        .post('/api/v1/donations/send')
        .send({ invalid: 'data' });
  ...
  describe('Non-Blocking Guarantee', () => {
    it('should never block requests even when detection errors', async () => {
    ...
    it('should process requests even when extreme patterns', async () => {
      // Extreme velocity
      const requests = [];
      for (let i = 0; i < 100; i++) {
        requests.push(
  ...
  describe('Success Resets Failure Counter', () => {
    it('should reset failure counter when successful request', async () => {
      // Failures
      for (let i = 0; i < 3; i++) {
        await request(app)
          .post('/api/v1/donations/fail')
