...
describe('API Versioning Support', () => {
    ...
    it('should negotiate version via Accept header correctly with vendor string', async () => {
        const res = await request(app).get('/stats/daily')
    ...
    it('should return 404 for unsupported API version in URL', async () => {
        const res = await request(app).get('/api/v999/stats/daily');
        expect(res.status).toBe(404);
        expect(res.body.error).toBe('Unsupported API version');
    });

    it('should return 404 for unsupported API version in Accept header', async () => {
        const res = await request(app).get('/stats/daily')
    ...
    it('should include X-API-Deprecated header and Sunset header for deprecated versions', async () => {
